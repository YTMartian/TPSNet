...
class TPSRecogHead(nn.Module, HeadMixin):
    ...
    def __init__(self, recognizer, num_fiducial=8, num_fiducial_gt=14, encoder=None,
        ...
        recognizer.update(max_seq_len=self.convertor.max_seq_len, start_idx=self.convertor.start_idx,
                          padding_idx=self.convertor.padding_idx)
    ...
    def forward(self, feat_maps, preds, p3_maps, p4_maps, p5_maps, gt_texts=None, polygons_area=None,
                lv_tps_coeffs=None, **kwargs):
        ...
        if preds is None:
            preds = [preds] * 3
        sample_grids, sample_gt_texts, sample_idxs, batch_index = multi_apply(self.sample_feature, feat_maps, preds,
                                                                              gt_maps, pad_polygon_areas, pad_gt_texts,
                                                                              downsample_rates)
    ...
    def simple_test(self, feature_maps, det_results, img_metas, rescale):
        ...
        for level in range(len(feature_maps)):
            ...
            aligned_feature, _ = self.tpsalign(feature_maps[level], grid.float(),
                                               torch.zeros(grid.shape[0], device=grid.device, dtype=torch.float32),
                                               None)
    ...
    def my_simple_test(self, feature_maps, det_results, img_metas, rescale):
        # boundaries = np.concatenate(boundaries)
        if self.with_coord:
            mask_features = []
            for i in range(len(feature_maps)):
