...
def tps_decode(preds,
    ...
    绘制文字区域分类和文字中心热力图
    '''
    # class_img = cls_pred[0:2].softmax(dim=0).data.cpu()
    # class_img = torch.argmax(class_img, 0, keepdim=True).permute(1, 2, 0).type(torch.uint8).numpy()
    # class_img = cv2.resize(class_img, (2448, 2448)) * 255
    # class_img = cv2.GaussianBlur(class_img, (0, 0), 35)
    # cv2.imshow('text class out', np.array(ToPILImage()(class_img)))
    # plt.figure(figsize=(2448, 2448), dpi=1)
    # sns.heatmap(class_img, xticklabels=False, yticklabels=False, cmap="viridis", cbar=False) # coolwarm,viridis,crest,Blues
    # plt.savefig("text class.png", bbox_inches='tight')
    #
    # center_img = cls_pred[2:4].softmax(dim=0).data.cpu()
    # center_img = torch.argmax(center_img, 0, keepdim=True).permute(1, 2, 0).type(torch.uint8).numpy()
    # center_img = cv2.resize(center_img, (2448, 2448)) * 255
    # center_img = cv2.GaussianBlur(center_img, (0, 0), 35)
    # cv2.imshow('text center out', np.array(ToPILImage()(center_img)))
    # plt.figure(figsize=(2448, 2448), dpi=1)
    # sns.heatmap(center_img, xticklabels=False, yticklabels=False, cmap="viridis", cbar=False)  # coolwarm,viridis,crest,Blues
    # plt.savefig("text center.png", bbox_inches='tight')
    # cv2.waitKey(0)
