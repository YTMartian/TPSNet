...
class TPSHead(HeadMixin, BaseModule):
    ...
    def __init__(self,
        ...
        loss['fiducial_type'] = fiducial_type
        ...
        self.decoder = TPS(self.num_fiducial, num_points=num_reconstr_points, grid_size=sample_size, fiducial_type=fiducial_type)
        ...
        if self.num_convs > 0:
            ...
            for i in range(self.num_convs):
                cls_convs.append(ConvModule(self.in_channels, self.in_channels, kernel_size=3, stride=1, padding=1, conv_cfg=conv_cfg if i < 3 else None, norm_cfg=norm, act_cfg=dict(type='ReLU')))
                reg_convs.append(ConvModule(self.in_channels, self.in_channels, kernel_size=3, stride=1, padding=1, conv_cfg=conv_cfg if i < 3 else None, norm_cfg=norm, act_cfg=dict(type='ReLU')))
