    ...
    def forward_single(self, pred, gt, downsample_rate=None, areas=None):
        ...
        if num_pos > 0 and (self.with_point_loss or self.with_BA):
            ...
            if self.with_BA:
                ...
                loss_ba = self.ba_loss(boder_pre[:, :self.num_sample // 2], boder_gt[:, :self.num_sample * 5 // 2], scale=downsample_rate) + \
                          self.ba_loss(boder_pre[:, self.num_sample // 2:], boder_gt[:, self.num_sample * 5 // 2:], scale=downsample_rate)
